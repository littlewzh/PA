...
static def_DHelper(B) {
  sword_t simm= (((s->isa.instr.b.imm11<<11)|s->isa.instr.b.imm4_1<<1)|s->isa.instr.b.imm10_5<<5)|s->isa.instr.b.imm12<<12;
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I     , addi);   //this alse exits some problems
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00101 11", U     , auipc);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11011 11", J     , jal);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11000 11", B     , branch);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 01100 11", R     , interger);
